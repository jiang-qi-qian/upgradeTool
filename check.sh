...
if [ "${SKIPCHECK}" == "true" ]; then
    echo "Skip checking dynamic information"
...
if [[ -f '/etc/default/sequoiasql-mysql' || -f '/etc/default/sequoiasql-mariadb' ]]; then
    ...
    do
        ...
        if [ "`ha_inst_group_list -u"${SDBUSER}" -p"${SDBPASSWD}" --name="${instgroup}"`" != "" ]; then
            ...
            # 跳过 c79-jiangqiqian3307
            SQLHOSTARRAY=(`ha_inst_group_list -u"${SDBUSER}" -p"${SDBPASSWD}" --name="${instgroup}" | grep -v "c79-jiangqiqian3307" | tail -n 2 | awk '{print $3}'`)
            SQLPORTARRAY=(`ha_inst_group_list -u"${SDBUSER}" -p"${SDBPASSWD}" --name="${instgroup}" | grep -v "c79-jiangqiqian3307" | tail -n 2 | awk '{print $4}'`)
